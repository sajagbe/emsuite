...
def load_surface_file(surface_file):
    """
    Load surface coordinates and charges from a pre-generated .surf file.
    
    This function requires a .surf file generated by the surface module
    (emsuite -s surface.in). The .surf file must have 4 columns: x, y, z, q.
    
    Args:
        surface_file (str): Path to the .surf file
        
    Returns:
        tuple: (surface_coords, surface_charges) where:
            - surface_coords: numpy array of shape [N, 3]
            - surface_charges: numpy array of shape [N]
            
    Raises:
        FileNotFoundError: If surface file does not exist
        ValueError: If surface file does not have 4 columns
        
    Note:
        Generate surface files using: emsuite -s surface.in
    """
    print(f"Loading surface data from {surface_file}...")
    coords, charges = load_surf(surface_file)
    print(f"Loaded {len(coords)} surface points with charges")
...
def normalize_effects(all_effects, effect_keys):
    ...
    Returns:
        tuple: (normalized_effects, normalization_params)
            - normalized_effects: List of dictionaries with normalized values
            - normalization_params: Dict with (min, max) for each key
    """
    normalized_effects = []
    ...
    for key in effect_keys:
        values = [effect.get(key, 0.0) for effect in all_effects if effect]
        if values:
            min_val = min(values)
            max_val = max(values)
        else:
...
def create_resume_metadata(logs_dir, calc_type, surface_file, total_points, 
    ...
    Args:
        ...
        surface_charge (float, optional): Surface charge value (deprecated, kept for compatibility)
    """
    metadata = {
...
def validate_resume_compatibility(metadata, current_params):
    ...
    # Check surface_file if available in both
    if 'surface_file' in metadata and 'surface_file' in current_params:
        if metadata['surface_file'] != current_params['surface_file']:
            checks.append(f"surface_file mismatch: {metadata['surface_file']} vs {current_params['surface_file']}")
...
def prompt_user_resume(logs_dir, metadata):
    ...
    print(f"Surface File:       {metadata.get('surface_file', 'N/A')}")
...
def main(tuning_file='tuning.in'):
    ...
    # Molecule parameter (required - must be XYZ file)
    molecule = tuning_params.get('molecule')
    if molecule is None:
        raise ValueError(
            "'molecule' parameter is required in tuning.in. "
            "Example: molecule = 'water.xyz'"
        )
    if not os.path.exists(molecule):
        raise FileNotFoundError(f"Molecule file not found: {molecule}")
    
    molecule_name = core.extract_xyz_name(molecule)
    
    # Surface file parameter (required - must be pre-generated .surf file)
    surface_file = tuning_params.get('surface_file')
    if surface_file is None:
        raise ValueError(
            "'surface_file' parameter is required in tuning.in. "
            "Generate a surface file first using: emsuite -s surface.in"
        )
    
    # QM calculation parameters
    ...
    # Load surface data from pre-generated .surf file
    surface_coords, surface_charges = load_surface_file(surface_file)
    ...
    if not resume_mode:
        ...
        create_resume_metadata(
            logs_dir, calc_type, surface_file, len(surface_coords),
            properties_to_calculate, 
            None  # Charges are stored in surf file
        )
    ...
    base_molecules = create_molecule_objects(
        molecule, basis_set, spin, method, functional, charge, 
    ...
    if calc_type == 'combined':
        ...
        # Use charges from surf file
        q_mm = surface_charges
        mean_charge = np.mean(surface_charges)
        ...
        try:
            ...
            log_point_result(logs_dir, 0, np.mean(surface_coords, axis=0), 
                           mean_charge, combined_effects, success=True)
            
            # Append to summary
            append_point_to_summary(summary_file, 0, np.mean(surface_coords, axis=0),
                                   mean_charge, combined_effects, success=True, total_points=1)
            ...
            # Log failure
            log_point_result(logs_dir, 0, np.mean(surface_coords, axis=0), 
                           mean_charge, None, success=False, error_msg=error_msg)
            
            # Append failure to summary
            append_point_to_summary(summary_file, 0, np.mean(surface_coords, axis=0),
                                   mean_charge, None, success=False, 
        ...
        finalize_summary_log(summary_file, all_effects, output_coords, [mean_charge])
        ...
        # Get charges from surf file (already per-point)
